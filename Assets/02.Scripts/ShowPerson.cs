...
public class ShowPerson : MonoBehaviour
{
    ...
    public void GenerateCustormer()
    {
        ...
        if (countStar == 1)
        {
            infoTextName.text = $"이름 : {(isAlien ? GenerateName() : name)}";

            infoTextINumText.text = "주민 번호 : ";
            infoTextINum.text = $"{(isAlien ? FalsifyINum() : INum)}";
            infoTextEtc.text = $"주소 : {(isAlien ? GenerateAddress() :address)}\n" +
                $"발급 일자 : {(isAlien ? GenerateIDate(INum.Substring(0, 8), isAlien) : pubDate)}\n" +
                $"발급 장소 : {(isAlien ? GenerateAddress() : address)}청";
    ...
    public void CheckCustomer(int type)
    {
        ...
        else if(type == 4) //외계인인지 체크
        {
            ...
            Debug.Log($"외계인 정답 수 : {GameManager.Instance.AlienCheck }");
            Debug.Log($"외계인 정답 수 : { GameManager.Instance.TotalAlien}");
        }
